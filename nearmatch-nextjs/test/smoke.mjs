...
const login = await request('/api/auth/login', {
  ...
  body: JSON.stringify({ email: 'priya@soundvision.in', password: 'nearmatch-demo' }),
});
...
const claim = await request('/api/claim', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ productId: 'sony-xb100', storeId: 's1' }),
});
assert.match(claim.pin, /^\d{4}$/, 'claim should return a four digit PIN');
const redeemed = await request('/api/redeem', {
  method: 'POST',
  headers: { ...headers, 'Content-Type': 'application/json' },
  body: JSON.stringify({ pin: claim.pin }),
});
assert.equal(redeemed.claim.status, 'Redeemed');
console.log('NearMatch (Next.js + MongoDB) smoke test passed: catalog, login, inventory, claim, redemption');
